...
type SectionKey = 'aspectRatio' | 'samplingMethod' | 'maskedContent' | 'maskingMode' | 'advanced';

const ImageSettings: React.FC<ImageSettingsProps> = ({ onSettingsChange }) => {
    ...
    <View style={styles.container}>
        ...
        {expandedSections.aspectRatio && (
          <View style={styles.sectionContent}>
            <View style={styles.optionsGrid}>
              {aspectRatioImages.map((image, index) => (
                <TouchableOpacity
                  ...
                  style={[styles.optionBox, settings.aspectRatio === aspectRatios[index] && styles.selectedOption]}
                  onPress={() => setSettings({ ...settings, aspectRatio: aspectRatios[index] })}
                >
            ...
            <View style={styles.ratioPreviewContainer}>
              <GridLines />
              <View style={[styles.ratioPreview, getRatioStyle(settings.aspectRatio)]} />
            </View>
          </View>
        )}
      </View>
      ...
      <View style={styles.section}>
        ...
        >
          ...
          <Ionicons
            name={expandedSections.samplingMethod ? "chevron-down" : "chevron-up"}
            size={20}
            color="#666666"
            style={styles.expandIcon}
          />
          ...
          <View style={styles.sectionContent}>
            <View style={styles.optionsGrid}>
              {samplingMethods.map((method) => (
                <TouchableOpacity
                  key={method}
                  style={[styles.optionBox, settings.samplingMethod === method && styles.selectedOption]}
                  onPress={() => setSettings({ ...settings, samplingMethod: method })}
                >
                  <Text style={[styles.buttonText, settings.samplingMethod === method && styles.selectedButtonText]}>
                    {method}
                  </Text>
                </TouchableOpacity>
              ))}
      ...
      <View style={styles.section}>
        ...
        >
          ...
          <Ionicons
            name={expandedSections.maskedContent ? "chevron-down" : "chevron-up"}
            size={20}
            color="#666666"
            style={styles.expandIcon}
          />
          ...
          <View style={styles.sectionContent}>
            <View style={styles.optionsGrid}>
              {maskedContentOptions.map((option) => (
                <TouchableOpacity
                  key={option}
                  style={[styles.optionBox, settings.maskedContent === option && styles.selectedOption]}
                  onPress={() => setSettings({ ...settings, maskedContent: option })}
                >
                  <Text style={[styles.buttonText, settings.maskedContent === option && styles.selectedButtonText]}>
                    {option}
                  </Text>
                </TouchableOpacity>
              ))}
          ...
          <View style={styles.sectionContent}>
            <View style={styles.optionsGrid}>
              {maskingModes.map((mode) => (
                <TouchableOpacity
                  key={mode}
                  style={[styles.optionBox, settings.maskingMode === mode && styles.selectedOption]}
                  onPress={() => setSettings({ ...settings, maskingMode: mode })}
                >
                  <Text style={[styles.buttonText, settings.maskingMode === mode && styles.selectedButtonText]}>
                    {mode}
                  </Text>
                </TouchableOpacity>
              ))}
          ...
          <View style={styles.sectionContent}>
            <View style={styles.section}>
              <SlideBar
                ...
                fixedPoint={1}
                isPercent={false}
                onValueChange={(value) => setSettings({ ...settings, cfgScale: value })}
              />
            </View>
            <View style={styles.section}>
              <SlideBar
                ...
                fixedPoint={2}
                isPercent={true}
                onValueChange={(value) => setSettings({ ...settings, denoisingStrength: value })}
              />
            </View>
          </View>
        )}
